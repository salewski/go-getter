...
// FIXME: This may not be true by the time I finish flip-flopping on the issue
// Therefore, if the
// 'src' param looks unambiguously like a filepath, it will be used "as is"
// (well, in its (mostly) Clean form).
//
...
// coupled with the relative filepath means the item definitely is for the
// function, but simply cannot be processed because no rooted path value was
// provided against which to resolve it. The function therefore emits an error
// directly.
//
...
func Test_detectGitForceFilepath_directly_pos_nonempty_srcResolveFrom(t *testing.T) {
	...
	// Our test loop below runs two iterations. Once with an empty 'pwd'
	// value, and once with a non-empty 'pwd' value.
	//
	// During both loops, our non-empty srcAbsResolveFrom values for the
	// 'srcResolveFrom' param are provided in addition to the value provided
	// for the 'pwd' param.
	//
	// The 'CtxDetect' protocol, ultimately implemented here in our
	// 'detectGitForceFilepath' function, is that 'src' is resolved relative
